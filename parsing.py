...
class Parsing():
    ...
    def valid_connection_value(self, values: str) -> Tuple[str, str]:
        ...
        if zone1 in self.graph.adjacency:
            for c in self.graph.adjacency[zone1]:
...
should be: nb_drones: <int>")
    ...
    def zone_checker(self) -> None:
        ...
        for line in self.lines[i:]:
            ...
            if key.strip() not in ['hub', 'end_hub', 'start_hub']:
                raise ParsingError(
                    "Invalid rule name! should be hub: <zone> <x> <y>")
    ...
    def connection_parser(self, line: str) -> None:
        ...
        if name1 not in self.graph.adjacency:
            self.graph.adjacency[name1] = []
        if name2 not in self.graph.adjacency:
            self.graph.adjacency[name2] = []
